...
public class DefaultOAuth2UserServiceTests {
	...
	@Test
	public void loadUserWhenUserInfoRequestSchemaIsNullThenThrowOAuth2AuthenticationException() {
		this.exception.expect(OAuth2AuthenticationException.class);
		this.exception.expectMessage(containsString("missing_user_info_request_schema"));

		when(this.userInfoEndpoint.getUri()).thenReturn("http://provider.com/user");
		when(this.userInfoEndpoint.getUserInfoRequestSchema()).thenReturn(null);
		this.userService.loadUser(new OAuth2UserRequest(this.clientRegistration, this.accessToken));
	}
	...
	public void loadUserWhenUserNameAttributeNameIsNullThenThrowOAuth2AuthenticationException() {
		...
		when(this.userInfoEndpoint.getUri()).thenReturn("http://provider.com/user");
		when(this.userInfoEndpoint.getUserInfoRequestSchema()).thenReturn(UserInfoRequestSchema.HEADER);
...
//			"}\n";		// Make the JSON invalid/malformed
	...
	public void loadUserWhenUserInfoRequestSchemaFormSuccessResponseThenHttpMethod() throws Exception {
		...
		when(this.userInfoEndpoint.getUri()).thenReturn(userInfoUri);
		when(this.userInfoEndpoint.getUserInfoRequestSchema()).thenReturn(UserInfoRequestSchema.FORM);
		when(this.userInfoEndpoint.getUserNameAttributeName()).thenReturn("user-name");
		when(this.accessToken.getTokenValue()).thenReturn("access-token");
